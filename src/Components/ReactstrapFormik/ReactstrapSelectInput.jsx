...
// const ReactstrapSelectInput = ({ field, form: { touched, errors, setFieldValue }, setvalue, ...props }) => {
...
//     searchInput ? setList(props.inputprops.options.filter((item) => item.name.toLowerCase().includes(searchInput?.toLowerCase()))) : searchInput;
//   }, [searchInput]);
...
//           value={t(setvalue !== undefined ? props.inputprops.value : props.inputprops?.options?.find((item) => item.id === field.value)?.name) || t("Select")}
//           onClick={() => {
//             setIsComponentVisible((prev) => prev !== props.inputprops.name && props.inputprops.name);
//           }}
//           readOnly
//           invalid={Boolean(touched[field.name] && errors[field.name])}
//         />
//         <Input id={props.inputprops.id} {...field} {...props} placeholder="Search" className="form-control form-select" type="text" invalid={Boolean(touched[field.name] && errors[field.name])} disabled />
//         {Boolean(props.inputprops.options?.length) && (
//           <div className={`box-content ${props.inputprops.name == isComponentVisible ? "open" : ""}`}>
...
//           </div>
//         )}
//         {touch && error && <FormFeedback>{t(props.title)} {t("IsRequired")}</FormFeedback>}
//         {props?.inputprops.close && <div className="close-icon"><RiCloseLine onClick={() => setFieldValue(field.name, '')} /></div>}
//       </div>
//     </>
//   );
// };
  ...
  const onSelectValue = (option) => {
    if (multiple && Array.isArray(field?.value)) {
      ...
      const index = temp.findIndex(
        (elem) => elem[getValuesKey] == option[getValuesKey]
      );
      if (index !== -1) {
        temp.splice(index, 1);
      } else {
      ...
      setvalue
        ? setvalue(inputprops, valueToSet, index, title)
  ...
  useEffect(() => {
    ...
    if (props.inputprops?.setsearch) {
      Array.isArray(field?.value) &&
        setSelectedItems &&
        setSelectedItems(
          listOpt.filter((elem) => field?.value?.includes(elem[getValuesKey]))
        );
    } else {
      Array.isArray(field?.value) &&
        setSelectedItems &&
        setSelectedItems(
          list.filter((elem) => field?.value?.includes(elem[getValuesKey]))
        );
    }
    ...
    if (props.inputprops?.setsearch) {
      !Array.isArray(field?.value) &&
        setSelectedItems &&
        setSelectedItems(
          listOpt.find((elem) => field?.value == elem[getValuesKey])
        );
    } else {
      !Array.isArray(field?.value) &&
        setSelectedItems &&
        setSelectedItems(
          list.find((elem) => field?.value == elem[getValuesKey])
        );
    }
  }, []);
    ...
    <>
      ...
      <div className="custom-select-box" ref={ref}>
        ...
        {props.inputprops?.setsearch
          ...
          : list?.length > 0 && (
              <div
                className={`box-content custom-select ${
                      ...
                      <Fragment key={index}>
                        {option?.data ? (
                          ...
                          <li onClick={() => onSelectValue(option)}>
                            ...
                            <p
                              className={`cursor ${
                                (selectedItems?.[index]?.[getValuesKey] ||
